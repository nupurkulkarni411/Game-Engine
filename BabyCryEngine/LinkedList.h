#ifndef LINKED_LIST_H
#define LINKED_LIST_H


template< class Type > class LinkedList
{
public:

	struct Element
	{
		Type *data;
		Element *next;
		Element *prev;

		Element(Type *element)
		{
			data = element;
			next = prev = NULL;
		}

        ~Element()
		{
			SAFE_DELETE( data );

			if( next )
				next->prev = prev;
			if( prev )
				prev->next = next;
		}
	};

	LinkedList()
	{
		m_first = m_last = m_iterate = m_temp = NULL;
		m_totalElements = 0;
	}

	~LinkedList()
	{
		Empty();
	}

	Type *Add( Type *element)
	{
		if( element == NULL)
			return NULL;

		if(m_first == NULL)
		{
			m_first = new Element( element );
			m_last = m_first;
		}
		else
		{
			m_last->next = new Element(element);
			m_last->next->prev = m_last;
			m_last = m_last->next;
		}

		m_totalElements++;

		return m_last->data;
	}

	//Insert new element just before next element
	Type *InsertBefore(Type *element, Element *nextElement)
	{
		m_temp = nextElement->prev;

		//This is case where we are inserting before first element
		if(m_temp == NULL)
		{
			m_first = new Element( element );
			m_first->next = nextElement;
			nextElement->prev = m_first;

			return m_first->data;
		}
		else 
		{
			m_temp->next = new Element( element );
			m_temp->next->prev = m_temp;
			m_temp->next->next = nextElement;
			nextElement->prev = m_temp->next;

			return m_temp->next->data;
		}
	}


	//Remove given element from linked list and destroy the data
	void Remove(Type **element)
	{
      m_temp = m_first;
	  while( m_temp != NULL )
	  {

		  if( m_temp->data == *element )
		  {
			  if( m_temp == m_first )
			  {
				  m_first = m_first->next;
				  if( m_first )
					  m_first->prev = NULL;
			  }
			  else if( m_temp == m_last )
			  {
				  m_last = m_last->prev;
				  if( m_last )
					  m_last->next = NULL;
			  }
			  else
			  {
				  m_temp->prev->next = m_temp->next;
				  m_temp->next->prev = m_temp->prev;
			  }

			  SAFE_DELETE( m_temp );
			  *element = NULL;
			  m_totalElements--;

			  return;
		  }

		  m_temp = m_temp->next;
	  }
	}

	//Destroy all the elements in linked list
	void Empty()
	{
		while(m_last != NULL)
		{
			m_temp = m_last;
			m_last = m_last->prev;
			SAFE_DELETE(m_temp);
		}

		m_first = m_last = m_iterate = m_temp = NULL;
		m_totalElements = 0;
	}

	//Removes all elements and clears there data 
	//Doesnt deletes data held by each element
	void ClearPointers()
	{
		while( m_last != NULL )
		{
			m_temp = m_last;
			m_temp->data = NULL;
			m_last = m_last->prev;
			SAFE_DELETE( m_temp );
		}
		m_first = m_last = m_iterate = m_temp = NULL;
		m_totalElements = 0;
	}

	void ClearPointer( Type **element )
	{
		m_temp = m_first;
		while( m_temp != NULL )
		{
			if( m_temp->data == *element )
			{
				if( m_temp == m_first )
				{
					m_first = m_first->next;
					if( m_first )
						m_first->prev = NULL;
				}
				else if( m_temp == m_last )
				{
					m_last = m_last->prev;
					if( m_last )
						m_last->next = NULL;
				}
				else
				{
					m_temp->prev->next = m_temp->next;
					m_temp->next->prev = m_temp->prev;
				}

				m_temp->data = NULL;

				SAFE_DELETE( m_temp );

				*element = NULL;

				m_totalElements--;

				return;
			}

			m_temp = m_temp->next;
		}
	}

	//Iterates through the linked list
	Type *Iterate(bool restart = false)
	{
		if( restart )
			m_iterate = NULL;
		else
		{
			if( m_iterate == NULL )
				m_iterate = m_first;
			else
				m_iterate = m_iterate->next;
		}

		if( m_iterate == NULL )
			return NULL;
		else
			return m_iterate->data;
	}

	Type *GetCurrent()
	{
		if( m_iterate != NULL )
			return m_iterate->data;
		else
			return NULL;
	}

	Type *GetFirst()
	{
		if( m_first != NULL )
			return m_first->data;
		else 
			return NULL;
	}

	Type *GetLast()
	{
		if( m_last != NULL )
			return m_last->data;
		else
			return NULL;
	}

	//Returns next element from given element
	Type *GetNext( Type *element )
	{
		m_temp = m_first;
		while( m_temp != NULL )
		{
			if( m_temp->data == element )
			{
				if( m_temp->next != NULL )
					return m_temp->next;
				else 
					return NULL;
			}

			m_temp = m_temp->next;
		}

		return NULL;
	}

	//Returns random element from linked list
	Type *GetRandom()
	{
		if( m_totalElement == 0 )
			return NULL;
		else if ( m_totalElement == 1 )
			return m_first->data;

		unsigned long element = rand() * m_totalElements / RAND_MAX;
		m_temp = m_first;
		for( unsigned long e = 0; e < element ; e++ )
			m_temp = m_temp->next;

		return m_temp->data;
	}
		//-------------------------------------------------------------------------
	// Returns the complete element (including its next and previous pointers).
	//-------------------------------------------------------------------------
	Element *GetCompleteElement( Type *element )
	{
		m_temp = m_first;
		while( m_temp != NULL )
		{
			if( m_temp->data == element )
					return m_temp;

			m_temp = m_temp->next;
		}

		return NULL;
	}

	//-------------------------------------------------------------------------
	// Returns the total number of elements in the linked list.
	//-------------------------------------------------------------------------
	unsigned long GetTotalElements()
	{
		return m_totalElements;
	}

private:
	Element *m_first; // First element in the linked list.
	Element *m_last; // Last element in the linked list.
	Element *m_iterate; // Used for iterating the linked list.
	Element *m_temp; // Used for temporary storage in various operations.

	unsigned long m_totalElements; // Total number of elements in the linked list.
};

#endif