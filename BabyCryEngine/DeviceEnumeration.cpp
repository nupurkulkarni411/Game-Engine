...
INT_PTR DeviceEnumeration::Enumerate( IDirect3D9 *d3d)
{
	...
	//Adapter is actually handle to physical graphics card
	...
	{
       //Get adapters supporting pixel format
		...
		{
			//Get all displaymodes supported by this adapter
			D3DDISPLAYMODE mode;
...
INT_PTR DeviceEnumeration::SettingsDialogProc( HWND dialog, UINT msg, WPARAM wparam, LPARAM lparam )
{
	switch( msg )
	{
		case WM_INITDIALOG:
		{
			...
			wchar_t version[16];
			wchar_t desc[256];
			wsprintf( version, L"%d", LOWORD( m_adapter.DriverVersion.LowPart ) );
			wsprintf( desc, L"%c", m_adapter.Description );
			Edit_SetText( GetDlgItem( dialog, IDC_DISPLAY_ADAPTER ),desc);
			...
			else
			{
				...
				if( m_windowed == false )
				{
					...
					wchar_t l_bpp[16];
					wsprintf( l_bpp, L"%d", m_displayModes->GetCurrent()->bpp);
					...
					while( m_displayModes->Iterate() )
						if( !ComboBoxContainsText( dialog, IDC_DISPLAY_FORMAT, l_bpp ) )
							ComboBoxAdd( dialog, IDC_DISPLAY_FORMAT, (void*)m_displayModes->GetCurrent()->mode.Format, l_bpp );
					...
					while( m_displayModes->Iterate() )
					{
						...
						{
							wsprintf( text, L"%d x %d", m_displayModes->GetCurrent()->mode.Width, m_displayModes->GetCurrent()->mode.Height );
					...
					while( m_displayModes->Iterate() )
					{
						...
						{
							wsprintf( text, L"%d Hz", m_displayModes->GetCurrent()->mode.RefreshRate );
		...
		case WM_COMMAND:
		{
			switch( LOWORD(wparam) )
			{
				case IDOK:
				{
					...
					m_settingsScript->SaveScript("DiagSettings.txt");
				...
				case IDC_COLOUR_DEPTH:
				{
					if( CBN_SELCHANGE == HIWORD(wparam) )
					{
						...
						while( m_displayModes->Iterate() )
						{
							if( m_displayModes->GetCurrent()->mode.Format == (D3DFORMAT)PtrToUlong( ComboBoxSelected( dialog, IDC_COLOUR_DEPTH ) ) )
							{
								wsprintf( res, L"%d x %d", m_displayModes->GetCurrent()->mode.Width, m_displayModes->GetCurrent()->mode.Height );
				...
				case IDC_RESOLUTION:
				{
					if( CBN_SELCHANGE == HIWORD(wparam) )
					{
						...
						while( m_displayModes->Iterate() )
						{
							if( (DWORD)MAKELONG( m_displayModes->GetCurrent()->mode.Width, m_displayModes->GetCurrent()->mode.Height ) == (DWORD)PtrToUlong( ComboBoxSelected( dialog, IDC_RESOLUTION ) ) )
							{
								wsprintf( refresh, L"%d Hz", m_displayModes->GetCurrent()->mode.RefreshRate );
...
//-----------------------------------------------------------------------------
void DeviceEnumeration::ComboBoxAdd( HWND dialog, int id, void *data, wchar_t *desc )
{
...
//-----------------------------------------------------------------------------
bool DeviceEnumeration::ComboBoxContainsText( HWND dialog, int id, wchar_t *text )
{
